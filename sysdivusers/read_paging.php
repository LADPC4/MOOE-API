...
$sysdivusers->set_property("roleguid",$data->roleguid) ;
$sysdivusers->set_property("searchname",$data->searchname) ;
...
// $sysdivusers->set_property("confirmed","N") ;
...
// query products
$stmt = $sysdivusers->readPaging($from_record_num, $records_per_page);
$num = $stmt->rowCount();
...
if($num>0){
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
        ...
        if ($row["gschool"]<>'') {
            $school->set_schguid($row["gschool"]) ;
            $sch=$school->readOne();
            $row["schdescription"]=$sch["schdescription"];
    ...
    // include paging
    $total_rows=$sysdivusers->count();
    $page_url="{$home_url}sysdivusers/read_paging.php?";
