...
class wfnextstatus {
	...
	private $conn;
	//declare table workflow
   	private $table_workflow = "ct_wfnextstatus";
	//auto generated
    public $lasterror=null;
	public  $PRIMARY_KEY = array('wfnsguid'=>'wfnsguid');
	private $FIELD_workflow = array('wfnsguid'=>'wfnsguid',  'workflow'=>'workflow', 'wfstatus'=>'wfstatus', 'rectimestamp'=>'rectimestamp');
	protected $FIELD_MODIFIED = array();
	protected $RESULT = array();
	protected static $FOREIGN_KEYS = array();

    //wfnsguid, workflow, wfstatus, rectimestamp

	// protected $wfnextstatus = null;
    ...
    public function set_property($pworkflow, $pArg='0') {
        //chek if array exists 
        if (isset($this->properties[$pworkflow])) {
            $this->properties[$pworkflow]=$pArg;
        } else {
            $newprop=array($pworkflow=>$pArg);
            $this->properties=array_merge($this->properties,$newprop);
        }	
	}
    public function get_property($pworkflow) { return (string) $this->properties[$pworkflow]; }


	// public function set_wfnextstatus($pArg='0') {
	// 	IF ( $this->wfnextstatus !== $pArg){
	// 		$this->wfnextstatus=$pArg; $this->FIELD_MODIFIED['wfnextstatus']=1;
	// 	}
	// }
	// public function set_workflow($pArg='0') {
	// 	IF ( $this->workflow !== $pArg){
	// 		$this->workflow=$pArg; $this->FIELD_MODIFIED['workflow']=1;
	// 	}
	// }
	...
	// public function get_wfnextstatus() { return (string) $this->wfnextstatus; }
	// public function get_workflow() { return (string) $this->workflow; }
...
function create(){
    ...
    $query = "INSERT INTO " . $this->table_workflow . " SET ".$this->createUpdFlds();
...
function update(){
   ...
   $query = "Update " . $this->table_workflow . " SET ".$this->createUpdFlds()."  where ".$this->createIDQry();
...
function readOne(){
   ...
   public function readPaging($from_record_num, $records_per_page){
            ...
            $tblprfx="a" ; 
            $query = "SELECT 
                      distinct a.workflow , b.description
                      
                FROM
                ct_wfnextstatus a inner join 
                ct_workflow b on a.workflow=b.wfguid 
    ...
    public function readSuccessions(){
        ...
        $tblprfx="a" ; 
        $query = "SELECT 
                  wfstatus, a.rectimestamp, b.name, b.validate
                  
            FROM
                    ct_wfnextstatus a inner join 
                    ct_wfstatus b on a.wfstatus=b.id
        ...
        $this->bind($stmt);
        // // bind variable values
        // $stmt->bindParam(":workflow", PDO::PARAM_STR);
        // $stmt->bindParam(":L2", $records_per_page, PDO::PARAM_INT);
...
public function Succsesioncount(){
    $query = "SELECT COUNT(*) as total_rows FROM " . $this->table_workflow . " where 
    workflow=:workflow";
...
public function count(){
    $query = "SELECT COUNT(*) as total_rows FROM " . $this->table_workflow . "";
