...
class sysrolemenus {
	...
	private $conn;
	//declare table gsysrole
   	private $table_sysrolemenus = "sysrolemenus";
	//auto generated
    public $lasterror=null;
	public  $PRIMARY_KEY = array('grolesid'=>'grolesid');
	private $FIELD_gsysrole = array('grolesid'=>'grolesid',  'gsysrole'=>'gsysrole', 'gsysmenuid'=>'gsysmenuid', 'rectimestamp'=>'rectimestamp');
	protected $FIELD_MODIFIED = array();
	protected $RESULT = array();
	protected static $FOREIGN_KEYS = array();

    //grolesid, gsysrole, gsysmenuid, gtimestamp

	// protected $sysrolemenus = null;
    ...
    public function set_property($pgsysrole, $pArg='0') {
        //chek if array exists 
        if (isset($this->properties[$pgsysrole])) {
            $this->properties[$pgsysrole]=$pArg;
        } else {
            $newprop=array($pgsysrole=>$pArg);
            $this->properties=array_merge($this->properties,$newprop);
        }	
	}
    public function get_property($pgsysrole) { return (string) $this->properties[$pgsysrole]; }


	// public function set_sysrolemenus($pArg='0') {
	// 	IF ( $this->sysrolemenus !== $pArg){
	// 		$this->sysrolemenus=$pArg; $this->FIELD_MODIFIED['sysrolemenus']=1;
	// 	}
	// }
	...
	// public function get_sysrolemenus() { return (string) $this->sysrolemenus; }
...
function create(){
    ...
    $query = "INSERT INTO " . $this->table_sysrolemenus . " SET ".$this->createUpdFlds();
...
function update(){
   ...
   $query = "Update " . $this->table_sysrolemenus . " SET ".$this->createUpdFlds()."  where ".$this->createIDQry();
...
function readOne(){
    ...
    $query = "SELECT
            ...
            FROM
                " . $this->table_sysrolemenus . " " . $tblprfx ." 
   ...
   public function readPaging($from_record_num, $records_per_page){
            ...
            $tblprfx="a" ; 
            $query = "select distinct a.gsysrole, b.roledescription
                      from sysrolemenus a inner join 
                  sysroles b on a.gsysrole=b.roleguid inner join 
                  sysmenus c on a.gsysmenuid=c.gsysmenuid
    ...
    public function readrolemenus(){
        ...
        $this->bind($stmt);
        // // bind variable values
        // $stmt->bindParam(":gsysrole", PDO::PARAM_STR);
        // $stmt->bindParam(":L2", $records_per_page, PDO::PARAM_INT);
...
public function readrolemenuscount(){
    $query = "SELECT COUNT(*) as total_rows FROM " . $this->table_sysrolemenus . " where 
    gsysrole=:gsysrole";
...
public function count(){
    $query = "SELECT COUNT(*) as total_rows FROM " . $this->table_sysrolemenus . "";
