...
include_once '../objects/vatmatrix.php';
...
// $vatmatrix->set_property("schguid",$data->schguid) ;
// $vatmatrix->set_property("confirmed","N") ;
...
// query products
$stmt = $vatmatrix->readMatrix($from_record_num, $records_per_page);
$num = $stmt->rowCount();
...
if($num>0){
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
 	...
 	$gsysmenuid_item=$row;
        ...
        $vattype=$row['vattype'];
        $matrix_itm[$trantype][$vattype] = array(
            'bir2306' => $row['bir2306'],
            'bir2307' => $row['bir2307'],
    ...
    array_push($gsysmenuid_arr["records"], $matrix_itm);
 
    // include paging
    $total_rows=$vatmatrix->count();
    $page_url="{$home_url}vatmatrix/read_matrix.php?";
