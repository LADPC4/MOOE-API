...
try {
    // if($data->es=='Y'){
        $papcodes->set_property("es",$data->es) ;
    // }
    // if($data->jhs=='Y'){
        $papcodes->set_property("jhs",$data->jhs) ;
    // }
    // if($data->shs=='Y'){
        $papcodes->set_property("shs",$data->shs) ;
    // }

} catch (Exception $e) {
//continue if no schoo assignment
}
$stmt = $papcodes->readPaging($from_record_num, $records_per_page);
$num = $stmt->rowCount();
...
if($num>0){
    ...
    // include paging
    $total_rows=$papcodes->count();
    $page_url="{$home_url}papcodes/read_paging.php?";
