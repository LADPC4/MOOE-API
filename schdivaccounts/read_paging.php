...
// $schacctejs->set_property("confirmed","N") ;
...
if ($data->orgtable!='S'){
    $orgtable="schooldivisions";
    $orgidfld="schdivid";
}

try {
        // if($data->es=='Y'){
            $schacctejs->set_property("es",$data->es) ;
        // }
        // if($data->jhs=='Y'){
            $schacctejs->set_property("jhs",$data->jhs) ;
        // }
        // if($data->shs=='Y'){
            $schacctejs->set_property("shs",$data->shs) ;
        // }
...
// query products
$stmt = $schacctejs->readPaging($orgtable, $orgidfld);
$num = $stmt->rowCount();
...
if($num>0){
    ...
    // include paging
    $total_rows=$schacctejs->count();
    $page_url="{$home_url}schacctejs/read_paging.php?";
