...
$mooerequests->set_property("schdivid",$data->schdivid) ;
if ($data->schguid!=""){
    $mooerequests->set_property("schguid",$data->schguid) ;
}
$mooerequests->set_property("roleguid",$data->roleguid) ;
...
// query products
$stmt = $mooerequests->readPending($from_record_num, $records_per_page);
$num = $stmt->rowCount();
...
if($num>0){
    ...
    // include paging
    $total_rows=$mooerequests->Pendingcount();
    $page_url="{$home_url}mooerequests/read_pending.php?";
