...
$mooedisbursements->set_property("es",$data->es) ;
$mooedisbursements->set_property("jhs",$data->jhs) ;
$mooedisbursements->set_property("shs",$data->shs) ;
if ((isset($data->disreferenceno))) {
   if ($data->disreferenceno!=""){
    $mooedisbursements->set_property("disreferenceno",$data->disreferenceno) ;
   }
    
 } 

// $mooedisbursements->set_property("confirmed","N") ;
...
// query products
$stmt = $mooedisbursements->readPrinted($from_record_num, $records_per_page);
$num = $stmt->rowCount();
...
if($num>0){
    ...
    // include paging
    $total_rows=0; //$mooedisbursements->count();
    $page_url="{$home_url}mooedisbursements/read_paging.php?";
