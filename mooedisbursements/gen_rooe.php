...
// query transactions 
$stmt = $mooedisbursements->genROOEMain();
$num = $stmt->rowCount();
...
if($num>0){
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
        ...
        $detstmt=$mooedisbursements->genROOEDetails($row["schdivid"],$data->fundyear,$row["slevel"]);
        ...
        $thistotalutuils=0;
        $uetstmt=$mooedisbursements->genROOEDivTotalUtils($row["schdivid"],$data->fundyear,$row["slevel"]);
        $unum = $uetstmt->rowCount();
        // echo$row["schdivid"]."|".$data->fundyear."|".$row["slevel"] ;
        if($unum>0){
